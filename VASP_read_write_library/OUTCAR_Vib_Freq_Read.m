...
% Need GREP function.
function VibFreqs = OUTCAR_Vib_Freq_Read(flpath) %[VibFreqs,VibEvecs] = VASPOutputVibModesParse(basefldr,flname)
if ~exist(flpath,'file'); VibFreqs = -1;return;end
...
if P.nfiles == 0; VibFreqs = -1;return;end

%% Read Vibrations
VibFreqs = zeros(P.lcount,1);
for i=1:P.lcount
    textLine = P.match{i};
    if ~isempty(strfind(textLine,searchstr1))
        StrWords = textscan(textLine,'%s');
        VibFreqs(i) = str2double(StrWords{1}{iwrd12});
    elseif ~isempty(strfind(textLine,searchstr2))
        StrWords = textscan(textLine,'%s');
        VibFreqs(i) = str2double(StrWords{1}{iwrd22})*1i;
    end
end
end